...
const Home = () => {
    ...
    const jmt = <svg className='text-slate-500 mt-[40px]' stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" height="300px" width="300px" xmlns="http://www.w3.org/2000/svg">
    ...
    const question = <svg className='text-slate-500 mt-[50px] mb-[34px]' stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="250px" width="250px" xmlns="http://www.w3.org/2000/svg">
    ...
    return (
        <div className='flex flex-row justify-center gap-[5rem]'>
            <Link to="/login" ><Home_Card icon={jmt} tag='맛집' /></Link>
            <Link to="/join" ><Home_Card icon={board} tag='게시판' /></Link>
            <Link to="/" ><Home_Card icon={question} tag='미정' /></Link>
        </div>
    );
}
